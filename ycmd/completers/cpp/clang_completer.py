...
class ClangCompleter( Completer ):
    ...
    return {
      ...
      'GetDocQuick'              : ( lambda self, request_data, args:
         self._GetSemanticInfo( request_data,
                                ...
                                func = 'GetDocsForLocationInFile',
                                response_builder = _BuildGetDocResponse ) ),
      'ReParse'                  : ( lambda self, request_data, args:
         self._ReParse( request_data ) ),
      'RemoveCache'              : ( lambda self, request_data, args:
         self._RemoveCache( request_data ) ),
    }


  def _LocationForGoTo( self, goto_function, request_data, reparse = False ):
    filename = request_data[ 'filepath' ]
    if not filename:
  ...
  def _ReParse( self, request_data ):
    filename = request_data[ 'filepath' ]
    if not filename:
      raise ValueError( INVALID_FILE_MESSAGE )

    with self._files_being_compiled.GetExclusive( filename ):
      diagnostics = self._completer.ReParse(
        ToCppStringCompatible( filename ),
  ...
  def _RemoveCache( self, request_data ):
    filename = request_data[ 'filepath' ]
    if not filename:
      filename = ''

    self._completer.DeleteCachesForFile( ToCppStringCompatible( filename ) )
